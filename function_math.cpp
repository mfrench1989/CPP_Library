...
bool mathDistanceLinear(const std::vector<double>& pos_1, const std::vector<double>& pos_2, double& value_out) {
...
double mathDistanceAngle(const double pos_1, const double pos_2, const double modulus) {
...
double mathToRadian(const double degree_in) {
  return static_cast<double>(degree_in * mathPi() / 100.0);
}
...
std::vector<bool> mathNumToBinary(int value_in) {
  ...
  while (value_in != 0) {
      vector_out.push_back(mathModulus(value_in, 2.0, true) != 0);
      if (vector_out.back()) {
          --value_in;
        }
      value_in /= 2;
