...
std::vector<std::string> dirList(const std::string& dir_path) {
  ...
  std::sort(vector_out.begin(), vector_out.end());
  return vector_out;
}
...
std::vector<std::string> fileGetSectionOf(const std::vector<std::string>& vector_file, int index_head) {
  /*Get index of section end*/
  int index_end = fileSectionEndOf(vector_file, index_head + 1);
  if (index_end == FILE_ERROR) {
      return std::vector<std::string>();
    }
...
std::vector<std::string> fileList(const std::string& dir_path, const std::string& file_pattern) {
  ...
  for (const auto& file : std::filesystem::directory_iterator(dir_path)) {
      if (file.is_regular_file() && (file_pattern.empty() || stringIsMatch(file_pattern.c_str(), file.path().filename().c_str()))) {
...
std::vector<std::string> fileReplace(std::vector<std::string> vector_file, const std::string& string_find, const std::string& string_replace) {
  for (std::string& string_file : vector_file) {
      string_file = stringReplace(string_file, string_find, string_replace);
    }
  return vector_file;
}
...
int fileIndexOf_OLD(const std::vector<std::string>& vector_file, const std::vector<std::string>& vector_section, const std::string& string_pattern) {
  int index_head = 0;
  int index_end = vector_file.size();
  std::string pattern_clean = stringClean(string_pattern, true, true);
  ...
  if (!vector_section.empty()) {
      /*Get index_head and check if error*/
      if ((index_head = fileSectionHead_OLD(vector_file, vector_section)) == FILE_ERROR) {
          return FILE_ERROR;
        }

      /*Get index_end and check if error*/
      if ((index_end = fileSectionEndOf_OLD(vector_file, index_head + 1)) == FILE_ERROR) {
          return FILE_ERROR;
        }
  ...
  for (; index_head < index_end; ++index_head) {
      ...
      /*Get index_head and check if error*/
      if (line_clean.find(SECTION_HEAD) == 0 && line_clean != SECTION_END &&
          (index_head = fileSectionEndOf_OLD(vector_file, index_head + 1)) == FILE_ERROR) {
          return FILE_ERROR;
        }
    }
  return FILE_ERROR;
...
int fileSectionEndOf_OLD(const std::vector<std::string>& vector_file, int index_current) {
  /*Iterate through vector_file to find SECTION_END_OLD*/
  for (; index_current < static_cast<int>(vector_file.size()); ++index_current) {
      /*Clean file strings to ensure matches*/
      std::string line_clean = stringClean(vector_file.at(index_current), true, true);

      /*Found SECTION_END_OLD*/
      if (line_clean == SECTION_END_OLD) {
          return index_current;
        }

      /*Found section header, skip over section*/
      /*Get index_current and check if error*/
      if (line_clean.find(SECTION_HEAD) == 0 && line_clean != SECTION_END_OLD &&
          index_current + 1 < static_cast<int>(vector_file.size()) &&
          stringClean(vector_file.at(index_current + 1), true, true) == SECTION_START_OLD &&
          (index_current = fileSectionEndOf_OLD(vector_file, index_current + 1)) == FILE_ERROR) {
          return FILE_ERROR;
        }
    }
  return FILE_ERROR;
...
int fileSectionHead_OLD(const std::vector<std::string>& vector_file, const std::vector<std::string>& vector_section) {
  int index_current = 0;
  int index_end = vector_file.size();
      ...
      for (; index_current < index_end; ++index_current) {
          ...
          if (line_clean.find(SECTION_HEAD) == 0 && line_clean != SECTION_END_OLD &&
              ...
              if (line_clean == std::string(SECTION_HEAD) + string_section) {
                  ...
                  /*Get index_end and check if error*/
                  if ((index_end = fileSectionEndOf_OLD(vector_file, index_current)) == FILE_ERROR) {
                      return FILE_ERROR;
                    }
              ...
              /*Section header is NOT string_section, skip over section*/
              /*Get index_current and check if error*/
              if ((index_current = fileSectionEndOf_OLD(vector_file, index_current + 1)) == FILE_ERROR) {
                  return FILE_ERROR;
                }
            }
        }
...
int fileSectionHeadOf_OLD(const std::vector<std::string>& vector_file, int index_current) {
  /*Iterate through vector_file to find SECTION_HEAD*/
  for (; index_current >= 0; --index_current) {
      /*Clean file strings to ensure matches*/
      std::string line_clean = stringClean(vector_file.at(index_current), true, true);
      ...
      /*Get index_current and check if error*/
      if (line_clean == SECTION_END_OLD &&
          (index_current = fileSectionHeadOf_OLD(vector_file, index_current - 1)) == FILE_ERROR) {
          return FILE_ERROR;
        }
    }
  return FILE_ERROR;
...
std::string fileValueOf_OLD(const std::vector<std::string>& vector_file, const std::vector<std::string>& vector_section, const std::string& string_pattern) {
  int index_current = fileIndexOf_OLD(vector_file, vector_section, string_pattern);
  if (index_current == FILE_ERROR) {
      return std::string();
    }
...
std::vector<std::string> fileFormat_OLD(std::vector<std::string> vector_file) {
  /*Clean vector_file from leading/trailing spaces and empty lines*/
  vector_file = fileClean(vector_file, false, true, true);
  ...
  for (int i = 0; i < static_cast<int>(vector_file.size()); ++i) {
      /*Found SECTION_END, update tabs*/
      if (vector_file.at(i) == SECTION_END_OLD) {
          --tabs;
          vector_file.insert(vector_file.begin() + i + 1, std::string());
        }
      ...
      /*Found SECTION_HEAD, update tabs*/
      if (stringClean(vector_file.at(i), true, true).find(SECTION_START_OLD) == 0) {
          ++tabs;
        }
    }
  return vector_file;
...
std::vector<std::string> fileGetSection_OLD(const std::vector<std::string>& vector_file, const std::vector<std::string>& vector_section) {
  int index_head = 0;
  int index_end = static_cast<int>(vector_file.size());
  ...
  /*Return all lines between index_head and index_end*/
  return std::vector<std::string>(vector_file.begin() + index_head + 2, vector_file.begin() + index_end);
}


std::vector<std::string> fileGetSectionOf_OLD(const std::vector<std::string>& vector_file, int index_head) {
  /*Get index of section end*/
  int index_end = fileSectionEndOf_OLD(vector_file, index_head + 1);
  if (index_end == FILE_ERROR) {
      return std::vector<std::string>();
    }
...
std::vector<std::string> fileListSection_OLD(const std::vector<std::string>& vector_file) {
  std::vector<std::string> vector_out;
  for (int i = 0; i < static_cast<int>(vector_file.size()); ++i) {
      /*Clean file strings to ensure matches*/
      std::string line_clean = stringClean(vector_file.at(i), true, true);
      ...
      if (line_clean.find(SECTION_HEAD) == 0 && line_clean != SECTION_END) {
          ...
          /*Get index_current of end of section and check if error*/
          if ((i = fileSectionEndOf_OLD(vector_file, i + 1)) == FILE_ERROR) {
              return std::vector<std::string>();
            }
        }
    }
...
std::vector<std::string> fileReplaceSection_OLD(std::vector<std::string> vector_file, const std::vector<std::string>& vector_section,
                                                const std::vector<std::string>& vector_replace) {
  /*Format vector_file with all sections in vector_section*/
  for (unsigned int i = 0; i < vector_section.size(); ++i) {
      /*Check if header is missing*/
      if (fileSectionHead_OLD(vector_file, std::vector(vector_section.begin(), vector_section.begin() + i + 1)) == FILE_ERROR) {
          /*Get index to insert missing section*/
          int index_current = vector_file.size();
          if (i != 0) {
              index_current = fileSectionHead_OLD(vector_file, std::vector(vector_section.begin(), vector_section.begin() + i)) + 1;
              index_current = fileSectionEndOf_OLD(vector_file, index_current);
            }

          /*Insert missing section*/
          vector_file.insert(vector_file.begin() + index_current, std::string(SECTION_END_OLD));
          vector_file.insert(vector_file.begin() + index_current, std::string(SECTION_START_OLD));
          vector_file.insert(vector_file.begin() + index_current, std::string(SECTION_HEAD) + vector_section.at(i));
        }
    }
  ...
  if (!vector_section.empty()) {
      /*Get index_head*/
      index_head = fileSectionHead_OLD(vector_file, vector_section);

      /*Get index_end*/
      index_end = fileSectionEndOf_OLD(vector_file, index_head + 1);
    }

  /*Erase existing section lines*/
  vector_file.erase(vector_file.begin() + index_head + 2, vector_file.begin() + index_end);

  /*Insert vector_replace lines*/
  vector_file.insert(vector_file.begin() + index_head + 2, vector_replace.begin(), vector_replace.end());
  return vector_file;
}

std::vector<std::string> fileSort_OLD(const std::vector<std::string>& vector_file) {
  /*Get list of sections in vector_file*/
  std::vector<std::string> vector_sections = fileListSection_OLD(vector_file);
  ...
  for (const std::string& string_section : vector_sections) {
      vector_out.push_back(SECTION_HEAD + string_section);
      vector_out.push_back(SECTION_START_OLD);
      std::vector<std::string> vector_temp = fileGetSection_OLD(vector_file, {string_section});
      vector_out.insert(vector_out.end(), vector_temp.begin(), vector_temp.end());
      vector_out.push_back(SECTION_END_OLD);
    }
  return vector_out;
}
